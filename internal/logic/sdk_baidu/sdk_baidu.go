...
func (s *sSdkBaidu) GetBaiduSdkConfList(ctx context.Context) ([]*sys_model.BaiduSdkConf, error) {
	...
	err := sys_dao.SysConfig.Ctx(ctx).Hook(daoctl.HookHandler).
...
func (s *sSdkBaidu) SaveBaiduSdkConf(ctx context.Context, info *sys_model.BaiduSdkConf, isCreate bool) (*sys_model.BaiduSdkConf, error) {
	...
	count, err := sys_dao.SysConfig.Ctx(ctx).Hook(daoctl.HookHandler).Count(sys_do.SysConfig{Name: s.sysConfigName})
	if count > 0 {
		_, err = sys_dao.SysConfig.Ctx(ctx).Hook(daoctl.HookHandler).Where(sys_do.SysConfig{Name: s.sysConfigName}).Update(sys_do.SysConfig{Value: jsonString})
	} else {
		_, err = sys_dao.SysConfig.Ctx(ctx).Hook(daoctl.HookHandler).Insert(sys_do.SysConfig{Name: s.sysConfigName, Value: jsonString})
	}
...
func (s *sSdkBaidu) DeleteBaiduSdkConf(ctx context.Context, identifier string) (bool, error) {
	...
	jsonString := gjson.MustEncodeString(newItems)
	if _, err = sys_dao.SysConfig.Ctx(ctx).Hook(daoctl.HookHandler).Where(sys_do.SysConfig{Name: s.sysConfigName}).Update(sys_do.SysConfig{Value: jsonString}); err != nil {
