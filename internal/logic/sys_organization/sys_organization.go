...
func (s *sSysOrganization) GetOrganizationList(ctx context.Context, parentId int64, IsRecursive bool) ([]*sys_entity.SysOrganization, int, error) {
	...
	err := sys_dao.SysOrganization.Ctx(ctx).Hook(daoctl.HookHandler).Where(sys_do.SysOrganization{ParentId: parentId}).Scan(&result)
...
func (s *sSysOrganization) SaveOrganizationInfo(ctx context.Context, info sys_model.SysOrganizationInfo) (*sys_entity.SysOrganization, error) {
	...
	cascadeDeep := 1
	if info.ParentId > 0 {
		result, err := sys_dao.SysOrganization.Ctx(ctx).Hook(daoctl.HookHandler).
	...
	if info.Id <= 0 {
		...
		_, err = sys_dao.SysOrganization.Ctx(ctx).Hook(daoctl.HookHandler).Insert(sys_do.SysOrganization{
			Id:          info.Id,
			Name:        info.Name,
		...
		result, err = sys_dao.SysOrganization.Ctx(ctx).Hook(daoctl.HookHandler).Where(sys_do.SysOrganization{Id: info.Id}).One()
		...
		_, err = sys_dao.SysOrganization.Ctx(ctx).Hook(daoctl.HookHandler).Where(sys_do.SysOrganization{Id: info.Id}).
...
func (s *sSysOrganization) GetOrganizationInfo(ctx context.Context, id int64) (*sys_entity.SysOrganization, error) {
	result, err := sys_dao.SysOrganization.Ctx(ctx).Hook(daoctl.HookHandler).Where(sys_do.SysOrganization{Id: id}).One()
...
func (s *sSysOrganization) DeleteOrganizationInfo(ctx context.Context, id int64) (bool, error) {
	...
	count, err := sys_dao.SysOrganization.Ctx(ctx).Hook(daoctl.HookHandler).Where(sys_do.SysOrganization{ParentId: id}).Count()
	...
	info := sys_entity.SysOrganization{}
	err = sys_dao.SysOrganization.Ctx(ctx).Hook(daoctl.HookHandler).
