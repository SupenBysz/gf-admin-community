...
func (s *sArea) GetAreaListByParentId(ctx context.Context, parentId int64) (*sys_model.AreaListRes, error) {
	...
	result, _ := daoctl.Query[*sys_entity.SysArea](sys_dao.SysArea.Ctx(ctx).Hook(daoctl.HookHandler), &sys_model.SearchParams{
...
func (s *sArea) GetAreaById(ctx context.Context, id int64) *sys_entity.SysArea {
	result, err := daoctl.GetByIdWithError[sys_entity.SysArea](sys_dao.SysArea.Ctx(ctx).Hook(daoctl.HookHandler), id)
...
func (s *sArea) GetAreaByCode(ctx context.Context, areaCode string) *sys_entity.SysArea {
	...
	if sys_dao.SysArea.Ctx(ctx).Hook(daoctl.HookHandler).Scan(&result, sys_do.SysArea{AreaCode: areaCode}) != nil {
		return nil
	}
	return &result
}
