...
func (s *sSysRole) QueryRoleList(ctx context.Context, info sys_model.SearchParams, unionMainId int64) (*sys_model.RoleListRes, error) {
	...
	result, err := daoctl.Query[*sys_entity.SysRole](sys_dao.SysRole.Ctx(ctx).Hook(daoctl.HookHandler), &info, false)
...
func (s *sSysRole) GetRoleById(ctx context.Context, id int64) (*sys_entity.SysRole, error) {
	result, err := daoctl.GetByIdWithError[sys_entity.SysRole](sys_dao.SysRole.Ctx(ctx).Hook(daoctl.HookHandler), id)
...
func (s *sSysRole) Save(ctx context.Context, info sys_model.SysRole) (*sys_entity.SysRole, error) {
	...
	err := sys_dao.SysRole.Transaction(ctx, func(ctx context.Context, tx gdb.TX) error {
		if roleInfo.Id == 0 {
			...
			count, err := sys_dao.SysRole.Ctx(ctx).Hook(daoctl.HookHandler).Where(sys_do.SysRole{Name: info.Name, UnionMainId: info.UnionMainId}).Count()
			if err != nil {
			...
			// 清除缓存
			_, err = sys_dao.SysRole.Ctx(ctx).Hook(daoctl.HookHandler).Insert(roleInfo)
	...
	err = sys_dao.SysRole.Ctx(ctx).Hook(daoctl.HookHandler).Where(sys_do.SysRole{Id: roleInfo.Id}).Scan(&roleInfo)
...
func (s *sSysRole) Delete(ctx context.Context, roleId int64) (bool, error) {
	...
	err := sys_dao.SysRole.Ctx(ctx).Hook(daoctl.HookHandler).Where(sys_do.SysRole{Id: roleId}).Scan(&info)
	...
	err = sys_dao.SysRole.Transaction(ctx, func(ctx context.Context, tx gdb.TX) error {
		_, err = sys_dao.SysRole.Ctx(ctx).Hook(daoctl.HookHandler).Delete(sys_do.SysRole{Id: roleId})
...
func (s *sSysRole) SetRoleMember(ctx context.Context, roleId int64, userIds []int64, makeUserUnionMainId int64) (bool, error) {
	roleInfo := sys_entity.SysRole{}
	err := sys_dao.SysRole.Ctx(ctx).Hook(daoctl.HookHandler).Where(sys_do.SysRole{Id: roleId}).Scan(&roleInfo)
...
func (s *sSysRole) GetRoleByUserIdList(ctx context.Context, userId int64) ([]*sys_entity.SysRole, error) {
	...
	err = sys_dao.SysRole.Ctx(ctx).Hook(daoctl.HookHandler).WhereIn(sys_dao.SysRole.Columns().Id, roleIds).Scan(&roleInfoArr)
