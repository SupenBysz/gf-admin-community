...
import (
	...
	"github.com/SupenBysz/gf-admin-community/utility/internal/utils"
...
func doMapConvert(value interface{}, recursive recursiveType, tags ...string) map[string]interface{} {
	...
	newTags := gconv.StructTagPriority
	switch len(tags) {
	case 0:
		// No need handling.
	case 1:
		newTags = append(strings.Split(tags[0], ","), gconv.StructTagPriority...)
	default:
		newTags = append(tags, gconv.StructTagPriority...)
	}
	...
	case map[interface{}]interface{}:
		for k, v := range r {
			dataMap[gconv.String(k)] = doMapConvertForMapOrStructValue(
	...
	default:
		...
		case reflect.Slice, reflect.Array:
			...
			for i := 0; i < length; i += 2 {
				if i+1 < length {
					dataMap[gconv.String(reflectValue.Index(i).Interface())] = reflectValue.Index(i + 1).Interface()
				} else {
					dataMap[gconv.String(reflectValue.Index(i).Interface())] = nil
				}
			}
...
func doMapConvertForMapOrStructValue(in doMapConvertForMapOrStructValueInput) interface{} {
	...
	case reflect.Map:
		...
		for _, k := range mapKeys {
			dataMap[gconv.String(k.Interface())] = doMapConvertForMapOrStructValue(
				doMapConvertForMapOrStructValueInput{
					IsRoot:          false,
		...
		for i := 0; i < reflectValue.NumField(); i++ {
			...
			if in.RecursiveOption || rtField.Anonymous {
				...
				case reflect.Map:
					...
					for _, k := range mapKeys {
						nestedMap[gconv.String(k.Interface())] = doMapConvertForMapOrStructValue(
							doMapConvertForMapOrStructValueInput{
								IsRoot:          false,
