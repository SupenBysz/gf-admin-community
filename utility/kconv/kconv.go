...
func StructWithError[T any](params interface{}, pointer T, mapping ...map[string]string) (res T, err error) {
	err = gconv.Scan(params, pointer, mapping...)
	return pointer, err
}
...
func doStruct(params interface{}, pointer interface{}, mapping map[string]string, priorityTag string) (err error) {
	...
	if priorityTag != "" {
		priorityTagArray = append(utils.SplitAndTrim(priorityTag, ","), gconv.StructTagPriority...)
	} else {
		priorityTagArray = gconv.StructTagPriority
	}
