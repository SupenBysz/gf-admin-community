...
type SysMemberLevelUserDao struct {
	...
	columns     SysMemberLevelUserColumns // columns contains all the column names of Table for convenient usage.
...
// sysMemberLevelUserColumns holds the columns for table sys_member_level_user.
var sysMemberLevelUserColumns = SysMemberLevelUserColumns{
	Id:               "id",
	UserId:           "user_id",
	ExtMemberLevelId: "ext_member_level_id",
...
func NewSysMemberLevelUserDao(proxy ...dao_interface.IDao) *SysMemberLevelUserDao {
	var dao *SysMemberLevelUserDao
	if len(proxy) > 0 {
		dao = &SysMemberLevelUserDao{
			group:       proxy[0].Group(),
			table:       proxy[0].Table(),
			columns:     sysMemberLevelUserColumns,
	...
	return &SysMemberLevelUserDao{
		group:   "default",
		table:   "sys_member_level_user",
		columns: sysMemberLevelUserColumns,
	}
}
...
func (dao *SysMemberLevelUserDao) Columns() SysMemberLevelUserColumns {
	return dao.columns
}
